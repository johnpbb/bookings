...
const PUBLIC_KEYS = [
  ...
  'payment_surcharge_enabled', 'payment_surcharge_label',
  'payment_surcharge_type', 'payment_surcharge_amount',
]
...
export async function GET(req: NextRequest) {
  ...
  if (map.egate_webhook_secret) map.egate_webhook_secret = '••••••••'
...
export async function POST(req: NextRequest) {
  ...
  for (const [key, value] of Object.entries(body)) {
    ...
    if ((key === 'egate_shared_secret' || key === 'egate_webhook_secret') && value === '••••••••') continue
